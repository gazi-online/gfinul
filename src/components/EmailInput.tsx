...
type EmailInputProps = Omit<React.InputHTMLAttributes<HTMLInputElement>, 'type' | 'value' | 'onChange'> & {
  label: string;
  value: string;
...
export const EmailInput = forwardRef<HTMLInputElement, EmailInputProps>(function EmailInput(
  ...
  const validEmail = hasValue && isValidEmail(trimmedValue);
  ...
  const hoverClass = getInteractiveInputClass(state);

  const describedBy = [resolvedError ? errorId : null, !resolvedError && hint ? hintId : null]
    .filter(Boolean)
    .join(' ') || undefined;
    ...
    <div className={`w-full ${className}`}>
      ...
      <div className="relative">
        <input
          ...
          className={`w-full rounded-2xl border px-4 py-3.5 pr-11 text-sm font-medium outline-none transition-all duration-200 ease-out placeholder:text-slate-400 caret-blue-600 dark:caret-blue-300 ${INPUT_STATE_CLASSES[state]} ${hoverClass}`}
        />
